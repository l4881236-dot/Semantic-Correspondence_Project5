...
def pixel_to_feat_xy(x_pix: float, y_pix: float, resize_to: int, Hf: int, Wf: int):
    # note: features correspond to a grid of size (Hf,Wf); for 512->64, scale=8
    x = (x_pix / resize_to) * Wf
    y = (y_pix / resize_to) * Hf
    # clamp
...
def evaluate_full():
    ...
    for pj in tqdm(pair_files, desc=f"Eval {SPLIT}"):
        ...
        for (xs, ys, xt, yt, ok) in zip(src_kps[:,0], src_kps[:,1], trg_kps[:,0], trg_kps[:,1], valid):
            ...
            x_f, y_f = pixel_to_feat_xy(float(xs), float(ys), RESIZE_TO, Hf, Wf)
